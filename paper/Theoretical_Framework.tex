...
\author{Author 1}
\affiliation{Mathematics and Computational Physics Group}
...
\begin{figure}[htbp]
  \centering
  % Artwork pending (W4). Uncomment when figures/grade_lattice.pdf is ready:
  % \includegraphics[width=0.65\columnwidth]{figures/grade_lattice}
  \fbox{\parbox{0.60\columnwidth}{\centering\vspace{1.8cm}%
    \small\textit{grade\_lattice.pdf --- W4 artwork pending}%
    \vspace{1.8cm}}}
...
\begin{table}[htbp]
\caption{Primitive operations Op1--Op10 with domains, ranges, and
  epistemic status. Labels: A = AXIOM, Th = THEOREM,
  V = VERIFIED, S = SUPPORTED, C = CONSISTENT, D+P = DEFINED + PARTIAL (\epistemic{OPEN}).}
\label{tab:ops}
\begin{ruledtabular}
\begin{tabular}{clp{3.5cm}p{2.8cm}l}
...
Op1  & Identity
  ...
  & $x$ (self)
  & A \\
Op2  & $\alpha$-shift: $(n,m)\mapsto(n{+}1,m)$
  & $\Gamma$
  & $\Gamma\setminus\{\mathrm{VACUUM}\}$ (for $n\geq 1$)
  & A \\
Op3  & $\beta$-shift: $(n,m)\mapsto(n,m{+}1)$
  & $\Gamma$
  & $\Gamma\setminus\{\mathrm{VACUUM}\}$
  & A \\
Op4  & Composition: $\mathrm{Op}_i\circ\mathrm{Op}_j$
  & $(\mathrm{Op}_i,\mathrm{Op}_j)$
  & $\mathrm{Op}_k$
  & A \\
Op5  & Inversion
  & $\Gamma\setminus\{\mathrm{VACUUM}\}$
...
\begin{table}[htbp]
\caption{SU(2) corrected results. FSS form $1/L+1/L^2$ (Task~J 4-pt); all values
...
Quantity & Value & Flag \\
\hline
$R_\infty[\mathrm{SU}(2)]$ (4-pt FSS $\beta{=}8.0$, $1/L+1/L^2$, 4-pt)
  & $\RinfSUtwo$
  & VALIDATED \\
$R[L=10]$, SU(2) ($\beta{=}8.0$, seed 203, $\tau_{\mathrm{int}}=1.43$)
  & $0.3400\pm0.00122$
  & VALIDATED \\
Haar-avg commutator norm $\bar{C}[\mathrm{SU}(2)]$
  & $\ChaarsUTwo$
  & VALIDATED \\
\end{tabular}
\end{ruledtabular}
...
\begin{table}[htbp]
\caption{SU(3) corrected results (autocorrelation-corrected errors;
  $\tau_{\mathrm{int}}$: $L=4\to1.34$, $L=6\to2.46$,
  $L=8\to3.53$, $L=10\to3.60$; seeds 200--203).
  All values from canonical table; plan v1.0 values superseded.}
\label{tab:SU3}
\begin{ruledtabular}
\begin{tabular}{p{5.5cm}ll}
Quantity & Value & Flag \\
\hline
$\beta$-scan $L=4,6,8$: $R$ range
  & $[0.384,\,0.420]$
  & VALIDATED \\
High-$\beta$ plateau
  & $R\approx 0.396$
...
$R(L=8)$
  ...
  & VALIDATED \\
FSS best fit
  & $1/L + 1/L^2$; $\chi^2/\mathrm{dof}=\ChiFSSSUthree$
  & VALIDATED \\
$R_\infty[\mathrm{SU}(3)]$ (autocorr-corrected, 4-pt)
  & $\RinfSUthree$
  & VALIDATED \\
$\kappa$-sweep $L=6$, $\beta=8.0$
  & $R$ decreases $0.408\to 0.377$, all $>0$
  & VALIDATED \\
$R(L=10)$, SU(3) (seed 203)
  & $\RSUthreeLten$
  & VALIDATED \\
$R(L=8)$ replica, SU(3) (seed 302)
  & $0.3918\pm0.00063$
  & RESOLVED: Consistent with main ensemble. \\
Haar-avg commutator norm $\bar{C}[\mathrm{SU}(3)]$
  & $\ChaarsUThree$
  & VALIDATED \\
...
\begin{figure}[htbp]
  \centering
  % \includegraphics[width=\columnwidth]{figures/fss_su2_su3}
  \fbox{\parbox{0.90\columnwidth}{\centering\vspace{1.8cm}%
    \small\textit{fss\_su2\_su3.pdf --- W4 artwork pending}%
    \vspace{1.8cm}}}
...
\begin{figure}[htbp]
  \centering
  % \includegraphics[width=0.75\columnwidth]{figures/r_ordering}
  \fbox{\parbox{0.70\columnwidth}{\centering\vspace{1.8cm}%
    \small\textit{r\_ordering.pdf --- W4 artwork pending}%
    \vspace{1.8cm}}}
...
% Figure pending (W4 task): figures/four_layer.pdf
\begin{figure}[htbp]
\centering
% \includegraphics{figures/four_layer.pdf}
...
\begin{table}[H]
\caption{Full Epistemic Status Inventory. Distinguishing between axiomatic, theoretical, and empirical components of the HGST E1--E7 framework.}
