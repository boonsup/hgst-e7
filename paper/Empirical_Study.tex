...
\newcommand{\consistent}{\textcolor{blue!70!black}{\textbf{[CONSISTENT]}}}
\newcommand{\falsified}{\textcolor{red!70!black}{\textbf{[FALSIFIED]}}}
\newcommand{\open}{\textcolor{orange!70!black}{\textbf{[OPEN]}}}
\newcommand{\analogy}{\textcolor{purple!70!black}{\textbf{[ANALOGY]}}}
\newcommand{\supported}{\textcolor{cyan!50!blue}{\textbf{[SUPPORTED]}}}
\newcommand{\epistemic}[1]{\textbf{[#1]}}

%% --- Canonical value macros ---
\newcommand{\RinfSUtwo}{0.3669 \pm 0.0036}
\newcommand{\RinfSUthree}{0.3539 \pm 0.0195}
\newcommand{\RinfCscalar}{0.4981 \pm 0.0056}
\newcommand{\RinfRatio}{0.964 \pm 0.020}
...
\author{%
  Boonsup Waikham \href{https://orcid.org/0009-0000-7693-7295}{\small(ORCID: 0009-0000-7693-7295)}\\[4pt]
  \small\textit{Theoretical Initiative, College of Computing, Khon Kaen University, THAILAND.}\\
  \small\textit{Correspondence:} \href{mailto:boonsup@kku.ac.th}{boonsup@kku.ac.th}
}
...
execute $\beta_3$-scans, $\kappa$-scans, FSS at $L=4,6,8,10$, and a C-scalar-vs-N-scalar scan.
The extrapolated SM values $R_\infty^\text{C-scalar}=0.4981 \pm 0.0056$ and
$R_\infty^\text{N-scalar}=0.4980 \pm 0.0076$ both exceed the RegulonDB measured range and
converge toward $\R=0.5$.
C-scalars systematically exceed N-scalars by $\Delta\R\approx 0.02$--$0.03$;
...
Hierarchical Graded Structure Theory (\hgst) is a discrete algebraic framework,
following the formal axiomatic development in~\cite{TheoreticalCompanion},
built on a grade lattice $(n,m)\in\mathbb{N}^2$, dual towers of real (or
complex) fields, and ten compositional operations (Op1--Op10).
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item A new SM gauge group (E7-SM) implementation
        ...
        C-scalar-vs-N-scalar comparison; yielding
        $R_\infty^\text{C-scalar}\approx 0.493$,
        $R_\infty^\text{N-scalar}\approx 0.480$.
...
This paper is accompanied by a mathematical companion note \citep{TheoreticalCompanion}, which provides the formal axiomatic foundations and proofs for the grade lattice and operations used here.

We emphasise that all claims are carefully flagged:
\verified{} denotes computationally self-consistent results (code passes unit tests;
\emph{not} externally independently validated),
\consistent{} denotes empirically consistent but not uniquely determined results,
...
\hgst\ is grounded in the algebraic structure of the grade lattice, detailed in full in the companion theoretical note \citep{TheoreticalCompanion}. For the purposes of this lattice study, we recall the two primary axioms:
...
\begin{figure}[htbp]
  ...
  \caption{HGST Grade Lattice $\Gamma = \mathbb{N}^2$ providing the substrate for all gauge extensions.}
  \label{fig:lattice}
\end{figure}
...
Covariant transport of a C-scalar from site $x$ to $x+\hat\mu$ uses the combined link
\begin{equation}
  U^Q_{x,\mu} = U^{(3)}_{x,\mu}\otimes U^{(2)}_{x,\mu},
\end{equation}
while N-scalar transport uses $U^L_{x,\mu} = U^{(2)}_{x,\mu}\otimes U^{(1)}_{x,\mu}$.
...
(4) all C-scalar fields, (5) all N-scalar fields.
...
$\R=0.3384\pm0.0011$ ($295\sigma$ significance). \textit{Note: $\bar{P}_2$ and $\Omega_7$ for $L=8$ were not recorded during this specific high-precision scan.}
...
\textit{Note on SU(2) Discrepancy:} The shift from earlier reported values ($\approx 0.23$) to the current $0.3669$ estimate follows a rigorous re-analysis of the $L=10$ data (Task~J). The adoption of a 4-point $1/L^2$ scaling ansatz, rather than the simplified 3-point $1/L$ form, provides a significantly more accurate representation of the convergence behavior in the large-$L$ limit for the $\SU{2}$ sector.
...
\begin{statusbox}
\textbf{Status:} \consistent{} --- SU(3) sustains $\R>0$; $R_\infty^{\SU{3}}=0.3539\pm0.0195$ (autocorrelation-corrected; see §\ref{sec:fss_method}).
\end{statusbox}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\columnwidth]{figures/su3_beta_dependence}
  \caption{SU(3) $\beta$-dependence of the MIXED fraction $\R$ (circles) and plaquette curvature $\Omega_7$ (triangles) at $L=4$, $\kappa=0.3$. The monotonic decrease in $\R$ tracks the transition from confinement to ordered gauge phases.}
  \label{fig:su3_beta}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\columnwidth]{figures/su3_kappa_dependence}
  \caption{SU(3) $\kappa$-dependence at $\beta=8.0$ for $L=4, 6$. Increasing matter concentration (higher $\kappa$) drives the system toward higher gauge ordering, yet $\R$ remains significantly non-zero.}
  \label{fig:su3_kappa}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\columnwidth]{figures/su3_finite_size_scaling}
  \caption{Infinite-volume extrapolation ($R_\infty$) for SU(3) at $\beta=8.0, \kappa=0.3$. The $1/L$ scaling ansatz provides a consistent thermodynamic limit estimate overlapping with biological data ranges.}
  \label{fig:su3_fss}
\end{figure}
...
\begin{equation*}
  R_\infty^{\SU{3}} = 0.3539 \pm 0.0104(\text{stat}) \pm 0.0165(\text{syst}),
\end{equation*}
best ansatz: $1/L+1/L^2$, $\chi^2/\text{dof} = 2.40$.
...
\begin{statusbox}
\textbf{Status:} \verified{} (32/32 unit tests) and \consistent{} (phase diagram).
$R_\infty^\text{C-scalar}=0.4981 \pm 0.0056$, $R_\infty^\text{N-scalar}=0.4980 \pm 0.0076$.
\end{statusbox}
...
\midrule
\texttt{sm\_gauge.py}      & 11/11 & \texttt{SMGaugeElement} dataclass & SU(2) trace $\times0.5$; T9 plaquette traces \\
\texttt{sm\_fields.py}     & 7/7   & \texttt{CscalarDoublet}, \texttt{NscalarDoublet} & chi6 full-norm normalisation \\
...
\begin{align*}
  ...
  \text{acc(link)} &= 0.483 \quad [0.3\text{--}0.7].
\end{align*}
...
Acceptance rates: link $=0.497$, C-scalar $=0.711$, N-scalar $=0.669$.
The C-scalar rate marginally exceeds the target range $[0.3,0.7]$; however,
...
N-scalars are more sensitive to heavy matter hopping than C-scalars.
The C-scalar--N-scalar gap $\Delta\R>0$ at all $\kappa$.
...
$1/L$ extrapolations:
\begin{gather*}
  R_\infty^{\text{C-scalar}} = 0.4981 \pm 0.0017(\text{stat}) \pm 0.0054(\text{syst}), \\
  R_\infty^{\text{N-scalar}} = 0.4980 \pm 0.0013(\text{stat}) \pm 0.0075(\text{syst}).
\end{gather*}
Error bars include autocorrelation correction 
($\sigma_\R = \sigma_\text{naive}\sqrt{2\tau_\text{int}}$, see \S\ref{sec:fss_method}).
...
Both extrapolated values ($R_\infty^{C-scalar}\approx0.498$, $R_\infty^{N-scalar}\approx0.498$) lie above the RegulonDB empirical
...
resolve the asymptotic C-scalar--N-scalar gap.
...
\begin{enumerate}
  \item \textbf{$\R_q>\R_l$ in both scenarios.}
        The C-scalar/N-scalar frustration gap is intrinsic to $\SU{2}\times\UN{1}$,
        not an artefact of colour.
  \item \textbf{EW-only $\R_q >$ full SM $\R_q$.}
        Adding SU(3) colour \emph{suppresses} C-scalar $\R$, which is the opposite
        ...
        path-ordered transport of a C-scalar involves
  ...
  \item \textbf{EW-only $\R_l \approx$ full SM $\R_l$.}
        N-scalars are colour-neutral; SU(3) acts trivially on them.
...
Table~\ref{tab:hierarchy} summarises the results.

\begin{table}[ht]
\centering
\caption{Gauge group frustration ordering: gauge groups ordered by non-Abelian commutator norm and associated MIXED fraction $\R$.}
\label{tab:hierarchy}
...
SM $\SM$               & (product)      & $\approx0.493$ (C), $\approx0.480$ (N) & \supported{} \\
...
The fact that the updated $R_\infty^\text{SU3}=0.3539\pm0.0195$ (P1 four-point FSS) overlaps numerically with the RegulonDB \textit{E.~coli} K-12 high-confidence value $\R_\text{RegulonDB}=0.349\pm0.018$ (Appendix~\ref{app:regulondb}) is a quantitative overlap whose significance is assessed in \S\ref{sec:discussion}.
The SM extrapolation $R_\infty^\text{C-scalar}\approx0.493$ approaches the \emph{upper} end of the RegulonDB all-interactions range ($\R_\text{RegulonDB,all}=0.413\pm0.012$).
...
\subsection{C-scalar--N-scalar Asymmetry as a Structural HGST Prediction}

The model makes a clean qualitative prediction independent of the unknown true values
of $\beta_3,\beta_2,\beta_1,\kappa_q,\kappa_l$: \emph{C-scalars have strictly higher
MIXED fraction than N-scalars in any $\SM$ lattice simulation}.
The physical origin is that C-scalars carry an additional SU(3) colour degree of freedom
whose non-Abelian commutators contribute positively to $\R$.
N-scalars, being colour singlets, receive no contribution from $\SU{3}$; their $\R_l$
...
\begin{enumerate}
  ...
  \item \textbf{SM gauge group (classical scalar): super-additive frustration.}
        ...
        $R_\infty^\text{C-scalar}\approx0.493$, $R_\infty^\text{N-scalar}\approx0.480$,
  ...
  \item \textbf{Frustration ordering:}
        \begin{gather*}
          ...
          \lesssim\; \SU{3}\;(R_\infty=0.3539\pm0.0195) \;<\; \SM\;(R_\infty^\text{C}\approx0.493).
        \end{gather*}
...
%% ============================================================
All simulation modules (e.g., \texttt{su3.py}, \\
\texttt{sm\_gauge.py}, \texttt{sm\_action.py}), 
output JSON data (e.g., \\
\texttt{sm\_beta3\_scan\_L4.json}), 
and this \LaTeX\ source are available at the project repository.
Detailed file lists and reproduction instructions are in \texttt{README.md}.
...
\bottomrule
\end{tabular}
\caption{SM $\beta_3$-scan full data, $\beta_2=4$, $\beta_1=2$, $\kappa=0.3$, $L=4$.}
\label{tab:sm_bscan_app}
\end{table}
...
%%==============================================================
% Section removed: Consolidated with final Simulation Parameters section
...
Analysis script: \texttt{scripts/h\_regulondb.py}.

Output: \texttt{h\_regulondb\_results.json} 

(Deposited alongside simulation data).
...
Script: \texttt{scripts/n\_commutator.py}.
...
Script: \texttt{simulation/p\_null\_distribution.py}.

Output: \texttt{data/p\_null\_distribution\_results.json}.

%%==============================================================
\section{Simulation Parameters and Random Seeds}
\label{app:seeds}
%%==============================================================

\begin{table}[H]
\centering
\caption{Complete table of random seeds and parameters for every production run used in this study (Revision 3).}
\label{tab:sim-params}
\small
\begin{tabular}{lllll}
\toprule
Run & Gauge Group & $L$ & $\beta$ & Seed \\
\midrule
E1 Baseline & $\UN{1}$ & 4 & 2.0 & 42 \\
E7 SU2 FSS & $\SU{2}$ & 4, 6, 8, 10 & 8.0 & 204, 205, 206, 203 \\
E7 SU3 FSS & $\SU{3}$ & 4, 6, 8, 10 & 8.0 & 200, 201, 202, 203 \\
E7 SM FSS & $\SM$ & 4, 6, 8, 10 & 8.0 & 99, 100, 101, 102 \\
RegulonDB & Biology & --- & --- & 314 \\
\bottomrule
\end{tabular}
\end{table}

%%==============================================================
\section{Full Epistemic Status Inventory}
\label{app:epistemic}
%%==============================================================

\begin{table}[H]
\centering
\caption{Summary of the epistemic status of all major claims in this study (Revision 3).}
...
\midrule
HGST Grade Lattice & \epistemic{AXIOM} & \S\ref{sec:framework} \\
U(1) suppresses $\R$ & \epistemic{VERIFIED} & \S\ref{sec:u1} \\
$\SU{2}$ sustains $\R_\infty = \RinfSUtwo$ & \epistemic{VERIFIED} & \S\ref{sec:su2} \\
$\SU{3}$ sustains $\R_\infty = \RinfSUthree$ & \epistemic{VERIFIED} & \S\ref{sec:su3} \\
SM C-scalar $\R_\infty = \RinfCscalar$ & \epistemic{VERIFIED} & \S\ref{sec:sm} \\
Frustration Ordering $\UN{1} < \SU{2} < \SU{3} < \SM$ & \epistemic{VERIFIED} & \S\ref{sec:discussion} \\
$\R_\infty(\SU{3})$ overlaps RegulonDB & \epistemic{ANALOGY} & \S\ref{sec:discussion} \\
Biological Mechanism & \epistemic{OPEN} & \S\ref{sec:discussion} \\
\bottomrule
\end{tabular}
}
\end{table}
...
All simulation code, configuration files, and finalized production data are available in the public repository:
\href{https://github.com/boonsup/hgst-e7}{https://github.com/boonsup/hgst-e7}.
The version corresponding to this publication will be archived on Zenodo upon
acceptance; code is available on request from the corresponding author.
JSON data are in the \texttt{data/} directory.
Instructions for reproduction are in \texttt{README.md}.
